<script lang="ts">
  // KI-Einstellungen als eigenes Modal (kein Browser-Dialog): Adresse des
  // Sprachmodells mit Preset-Knöpfen, Modell-Auswahl und Status-Zeile mit
  // Prüfen-Knopf. Änderungen werden persistiert und lösen eine neue Prüfung aus.
  ...
  // Beim Öffnen den aktuellen Wert in den Entwurf spiegeln.
  $effect(() => {
    if (offen) entwurfUrl = kiEinstellungen.basisUrl
  })

  // Letztes Segment der Host-IP der drei Presets (im Standard-Heimnetz).
  const PRESETS = ['49', '50', '51'] as const
  ...
  function setzePreset(segment: string): void {
    ...
    try {
      ...
      // Kein gültiger URL-Wert - sauberen Standard aufbauen.
...
<Modal titel="KI-Einstellungen" bind:offen {onSchliessen}>
  ...
  <div class="hinweis-text">
    Ohne erreichbares Sprachmodell bleibt die App vollständig nutzbar - die KI-Funktionen sind dann
    ausgegraut.
  </div>
  {#snippet fuss()}
