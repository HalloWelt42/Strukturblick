...
- Werkzeuge: Abfrage (JSONPath, XPath, Volltext, regulärer Ausdruck), Schema ableiten und validieren (JSON Schema, XSD), Konvertieren mit Verlustbericht, Reparatur als Vorschau, Code erzeugen (TypeScript, Pydantic, Dataclasses, PHP 8.4+) und ein Testdaten-Generator (aus dem Dokument eine bearbeitbare Erzeuger-Spezifikation ableiten, dann beliebig viele deterministische Datensätze mit festem Startwert erzeugen und als NDJSON, JSON, CSV oder YAML ausgeben).
