"""Emitter für Pydantic-v2-Modelle aus dem neutralen Zwischenmodell."""
...
def emittiere_pydantic(modell: SchemaModell) -> str:
    """Erzeugt Pydantic-v2-Modelle (eine BaseModel-Klasse je benanntem Typ)."""
    zeilen: list[str] = [
    ...
    for typ in modell.typen:
        zeilen.append("")
        zeilen.append("")
        zeilen.append(f"class {typ.name}(BaseModel):")
        if not typ.felder:
            zeilen.append("    pass")
            continue
